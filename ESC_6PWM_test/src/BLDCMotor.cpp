  ...
  switch (controller) {
    ...
    case MotionControlType::velocity:
      ...
      if(torque_controller == TorqueControlType::voltage){
        ...
        if(!_isset(phase_resistance))  voltage.q = target;//current_sp;
#endif
...
void BLDCMotor::setPhaseVoltage(float Uq, float Ud, float angle_el) {
  ...
  //when target velocity is zero then only 5% of Ua, Ub, Uc will be delivered to the motor.
  if(target == 0.0f){
    Ua = Ua*0.05;
    Ub = Ub*0.05;
    Uc = Uc*0.05;
  ...
  // Serial.print(target);
  // Serial.print(",");Serial.print(Ua);
  // Serial.print(",");Serial.print(Ub);
  // Serial.print(",");Serial.println(Uc);
