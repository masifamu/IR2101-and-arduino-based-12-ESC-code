...
float FOCMotor::shaftAngle() {
  // static long prev_time_stamp;
  // // get current timestamp
  // unsigned long now_us = _micros();
  // // calculate the sample time from last call
  // float Ts = (now_us - prev_time_stamp) * 1e-6;
  ...
  // Serial.println(sensor->getAngle()+shaft_velocity*Ts);
  // save timestamp for next call
  // prev_time_stamp = now_us;

  return sensor_direction*LPF_angle(sensor->getAngle()) - sensor_offset;
  // return sensor_direction*LPF_angle(sensor->getAngle()+shaft_velocity*Ts) - sensor_offset;
}
