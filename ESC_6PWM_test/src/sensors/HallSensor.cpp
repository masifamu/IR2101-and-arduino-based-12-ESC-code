...
void HallSensor::updateState() {
  ...
  // Serial.print(electric_rotations);Serial.print("\t");
  // Serial.println(electric_sector);
...
float HallSensor::getAngle() {
  ...
  static long last_electrical_rotations;
  static int8_t last_electric_sector;
  if(last_electrical_rotations == electric_rotations && last_electric_sector == electric_sector){
    return ((electric_rotations * 6 + electric_sector) / cpr) * _2PI + 0.05f;
  }
  last_electrical_rotations = electric_rotations;
  last_electric_sector = electric_sector;
#endif
